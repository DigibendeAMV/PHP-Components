...
use Digicademy\Exception as Exception;
use XMLParser ;

/**
 * Class ContentParser
 * 
 * Parses DigiCademy Repository Document XHTML content and generates html for it.
 * 
 ...
 * what ever HTML the element finds approriate.
 * 
 * 
...
class Parser
{
    ...
    /**
     * Creates a new instance of the content parser
     *
     * @param string $file
     * @throws Exception When file does not exist
     */
    public function __construct(string $file, bool $verbose = false)
    {
        if (!file_exists($file)) {
    ...
    public function startParse(): string
    {
        ...
        xml_set_element_handler($xmlParser, 'elementStartHandler', 'elementEndHandler');
    ...
    // -----------------------------------------------------------------------------
        ...
        protected function elementStartHandler(XmlParser $xmlParser, string $name, array $attribs): bool
        {
            ...
            array_push($this->_stack, Element::factory($name, $attribs));
            $element = end($this->_stack);
